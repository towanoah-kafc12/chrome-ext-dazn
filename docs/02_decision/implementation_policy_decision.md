...
## 2026-06-18: ヘッダーは通常非表示、画面上部ホバー時のみ表示する

- 項番: 8
- 内容:
  - DAZNのヘッダーは通常時に画面外へ隠し、画面上部へマウスを移動したときだけ短時間表示する。
  - 動画プレイヤーを画面最上部から表示するため、ヘッダー高さは動画サイズ計算から常に除外する。
  - DAZN側がヘッダー分としてページ本体に残す `margin-top` / `padding-top` は、拡張適用中に打ち消す。
  - ただし完全な 0px では詰まりすぎるため、プレイヤー上部には 12px の余白を残す。
  - DAZN側の動画セクション左右paddingは打ち消し、プレイヤーを横幅いっぱいに近づける。
  - ホバー表示中のヘッダーは動画の上に重ねる。
- 判断理由:
  - ユーザ要望として、デフォルトではヘッダーとその下の空白をなくし、視聴領域を最大化したいため。
  - ヘッダーをレイアウト内に残すと、動画プレイヤーの上に空白が残るため。
...
## 2026-06-18: 右側固定バーとホバー文字列は存在ごと消す

- 項番: 9
- 内容:
  - DAZN再生ページ右端の固定バー、開閉ボタン、ホバー時の文字列は、視聴時に不要なUIとして非表示にする。
  - `[data-test-id="SIDE_BAR"]` は表示領域として確保せず、拡張適用中は `display: none` と幅0でレイアウトから外す。
  - `main__expand-match-day-panel-container` / `main__expandButtonTooltip` 系の右端固定ボタンとtooltipも、拡張適用中は非表示にする。
  - サイドバー幅を初回測定して固定する処理は不要になったため削除する。
- 判断理由:
  - ユーザ要望として、右側バーとホバー文字列は「不要」であり、動画に重ねる・幅計算から除外するだけではなく存在を消したい対象になったため。
